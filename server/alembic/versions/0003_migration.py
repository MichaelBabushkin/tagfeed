...
Revision ID: 757b821e9b13
Revises: d569340cf589
Create Date: 2024-04-23 01:34:15.335373
...
def downgrade() -> None:
    ...
    op.add_column(
        "items",
        sa.Column(
            "preview", sa.VARCHAR(length=200), autoincrement=False, nullable=True
        ),
    )
    ...
    op.execute(tcr.update().where(tcr.c.item_type == "TEXT").values(item_type="STRING"))
    op.execute(
        tcr.update().where(tcr.c.item_type == "IMAGE").values(item_type="NOT_DEFINED")
    )
